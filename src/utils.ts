...
export const getCardImage = (
  ...
  if (cardText.isPreviewText && preview?.image) {
    ...
    const imageTags = getImgsFromHTMLString(text)

    if (imageTags.length > 0) {
      fileSrc = imageTags[0]?.getAttribute('src')
      /**
       * End of backward compatibility code
       */
    } else {
      const doc = parse(text)
      const fileTags = doc.querySelectorAll('div[data-file]')
      ...
      if (videoFile) {
        ...
        fileSrc = !fileTags[0]
          ?.getAttribute('data-fileType')
          ?.startsWith('audio')
          ? fileTags[0]?.getAttribute('data-url')
          : undefined
      }
    }
  }
