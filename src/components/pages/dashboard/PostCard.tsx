...
function PostCard({
  ...
  const { mutate, isLoading } = useMutation(
    (data: { userId: string }) => {
      return http.post(`/bookMarkPost/${_id}`, data)
    },
    {
      onSuccess: _data => {
  ...
  const deletePostMutation = useMutation(
    () => {
      return http.delete(`/deletePost/${_id}`)
    },
    {
      onSuccess: () => {
  ...
  const introductoryPost = () => {
    introductoryPostMutation.mutate({ postId: _id })
  }
    ...
    >
      <div className='flex flex-col gap-2'>
        ...
        <div className='flex flex-row items-center gap-2'>
          ...
          <Typography
            type='body'
            className='text-grayMd font-[400] text-sm md:text-base !leading-[19px]'
          >
            {upvotes.length} Upvoted
          </Typography>
          ...
          {/* Separator */}
          <div className='hidden mobile:block w-[0.3rem] h-[0.3rem] rounded-full bg-grayLight' />
        ...
        >
          ...
          {postDropdownOpen ? (
            ...
            >
              ...
              selectedAccount?.introductoryPost !== _id ? (
                <button
                  id='intro-post-btn'
                  className={`w-full text-base font-[500] leading-[120%] text-gray5 bg-white h-12 md:h-16 rounded-[8px_8px_0_0] px-4 hover:bg-lightPurple py-2`}
              ...
              <button
                id='delete-post-btn'
                className={`w-full text-base font-[500] leading-[120%] text-gray5 bg-white h-12 md:h-16 rounded-[0_0_8px_8px] px-4 hover:bg-lightPurple py-2`}
                onClick={deletePost}
              >
                Delete This Post
              </button>
